...
mutacion::mutacion(const string & str){
	...
	if(CLNDSDBID.size() < CLNDBN.size()){
		for(unsigned int i = CLNDSDBID.size(); i < CLNDBN.size(); ++i)
			CLNDSDBID.push_back("");
	}

	if(CLNDSDB.size() < CLNDBN.size()){
		for(unsigned int i = CLNDSDB.size(); i < CLNDBN.size(); ++i)
			CLNDSDB.push_back("");
	}
    ...
    for(unsigned int i = 0 ; i < CLNDBN.size(); ++i)
...
ostream& operator<< ( ostream& os, const mutacion& m) {
    ...
    for(unsigned int i = 0; i < m.getGenes().size() ; ++i)
        os << m.getGenes().at(i) << " ";
        
    os << endl;
    
    for(unsigned int i = 0; i < m.getRef_alt().size() ; ++i)
        os << m.getRef_alt().at(i) << " ";
    
    os << endl;
    
   for(unsigned int i = 0; i < m.getClnsig().size() ; ++i)
        os << m.getClnsig().at(i) << " "; 
        
    os << endl;

    for(unsigned int i = 0; i < m.getEnfermedades().size() ; ++i)
        os << m.getEnfermedades().at(i) << " ";
    
	 os << endl;

    for(unsigned int i = 0; i < m.getCaf().size() ; ++i)
        os << m.getCaf().at(i) << " ";
